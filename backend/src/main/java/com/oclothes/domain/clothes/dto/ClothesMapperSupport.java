...
public class ClothesMapperSupport {
    ...
    public Closet findByClosetId(Long id) {
        return this.closetRepository.findByIdAndUser(id, getLoggedInUser())
                .orElseThrow(ClosetNotFoundException::new);
    }

    @Named("mapToTagDto")
    public <T extends ClothesTag<? extends Tag>> Set<TagDto.Response> mapToTagDto(Set<T> tags) {
        return tags.stream().map(tag -> new TagDto.Response(tag.getTag().getId(), tag.getTag().getName()))
